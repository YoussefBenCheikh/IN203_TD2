...
if (rank==0) {
	...
	MPI_Send(&jeton, 1, MPI_INT, 4, 1, MPI_COMM_WORLD);
	...
	MPI_Send(&jeton, 1, MPI_INT, 5, 1, MPI_COMM_WORLD);
	std::cout<<"jeton: "<<jeton<<" affiché par tâche: "<<rank<<"\n";
} else if (rank==2){
	int jeton;
	MPI_Recv(&jeton, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, stats);
	MPI_Send(&jeton, 1, MPI_INT, 6, 1, MPI_COMM_WORLD);
	std::cout<<"jeton: "<<jeton<<" affiché par tâche: "<<rank<<"\n";
} else if (rank==3) {
	int jeton;
	...
	int jeton;
	MPI_Recv(&jeton, 1, MPI_INT, 1, 1, MPI_COMM_WORLD, stats);
	MPI_Send(&jeton, 1, MPI_INT, 7, 1, MPI_COMM_WORLD);
	std::cout<<"jeton: "<<jeton<<" affiché par tâche: "<<rank<<"\n";
} else if (rank==6) {
	int jeton;
	MPI_Recv(&jeton, 1, MPI_INT, 2, 1, MPI_COMM_WORLD, stats);
	std::cout<<"jeton: "<<jeton<<" affiché par tâche: "<<rank<<"\n";
} else {
	int jeton;
	MPI_Recv(&jeton, 1, MPI_INT, 5, 1, MPI_COMM_WORLD, stats);
	std::cout<<"jeton: "<<jeton<<" affiché par tâche: "<<rank<<"\n";
}
